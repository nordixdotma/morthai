...
import { facialCareData } from "@/lib/services-data"
      ...
      <section className="service-section py-16 md:py-24 bg-[#fff8f5] rounded-t-xl md:rounded-t-3xl">
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="mt-12 space-y-16 md:space-y-20">
            {facialCareData?.map((facial, index) => {
              const isEven = index % 2 === 0
              const title = language === "fr" ? facial.title.fr : facial.title.en
              const description = language === "fr" ? facial.description.fr : facial.description.en

              return (
                <div key={facial.id} className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
                  <div
                    className={`relative h-80 md:h-96 rounded-lg overflow-hidden shadow-lg ${!isEven ? "order-2 md:order-2" : "order-1 md:order-1"}`}
                  >
                    <Image
                      src={facial.mainImage || "/placeholder.svg?height=500&width=500&query=facial"}
                      alt={title}
                      fill
                  ...
                  {/* Text Content */}
                  <div className={`space-y-4 ${!isEven ? "order-1 md:order-1" : "order-2 md:order-2"}`}>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mt-4">
                        ...
                        <Link
                          href={`/services/${facial.id}?offer=true`}
